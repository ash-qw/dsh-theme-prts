...
test('renders user and assistant as directional chat bubbles with speaker avatars', async ({ page }) => {
  ...
  const sessionFlowControls = appearance.locator('[data-prts-setting-row="sessionFlow"]')
  const activeLifeline = page.locator('[data-prts-session-row][aria-selected="true"] [data-prts-session-lifeline]')
  await expect(root).toHaveAttribute('data-prts-session-flow', 'on')
  await expect(activeLifeline).toHaveCSS('opacity', '0.98')
  await sessionFlowControls.getByRole('button', { name: '关闭' }).click()
  await expect(root).toHaveAttribute('data-prts-session-flow', 'off')
  await expect(activeLifeline).toHaveCSS('opacity', '0')
  await sessionFlowControls.getByRole('button', { name: '开启' }).click()
  await expect(root).toHaveAttribute('data-prts-session-flow', 'on')
  await expect(activeLifeline).toHaveCSS('opacity', '0.98')

  const sessionGlowControls = appearance.locator('[data-prts-setting-row="sessionGlow"]')
  const activeCoreFilament = activeLifeline.locator('[data-prts-session-lifeline-filament="core"]')
  const activeCoreHalo = activeCoreFilament.locator('[data-prts-session-lifeline-layer="halo"]')
  const activeCoreBody = activeCoreFilament.locator('[data-prts-session-lifeline-layer="body"]')
  const activeCoreLight = activeCoreFilament.locator('[data-prts-session-lifeline-layer="core"]')
  await expect(root).toHaveAttribute('data-prts-session-glow', 'on')
  await expect(activeCoreHalo).toHaveCSS('stroke-width', '4.25px')
  await expect(activeCoreBody).toHaveCSS('stroke-width', '1.65px')
  await expect(activeCoreLight).toHaveCSS('opacity', '0.96')
  await sessionGlowControls.getByRole('button', { name: '关闭' }).click()
  await expect(root).toHaveAttribute('data-prts-session-glow', 'off')
  await expect(activeCoreHalo).toHaveCSS('opacity', '0')
  await expect(activeCoreBody).toHaveCSS('stroke-width', '0.9px')
  await expect(activeCoreLight).toHaveCSS('opacity', '0')
  await sessionGlowControls.getByRole('button', { name: '开启' }).click()
  await expect(root).toHaveAttribute('data-prts-session-glow', 'on')
  await expect(activeCoreLight).toHaveCSS('opacity', '0.96')

  const sessionPaletteControls = appearance.locator('[data-prts-setting-row="sessionFlowPalette"]')
  const originalFlowStroke = await activeCoreBody.evaluate(node => getComputedStyle(node).stroke)
  await expect(root).toHaveAttribute('data-prts-session-flow-palette', 'triad')
  await sessionPaletteControls.getByRole('button', { name: '金黄' }).click()
  await expect(root).toHaveAttribute('data-prts-session-flow-palette', 'amber')
  await expect.poll(() => activeCoreBody.evaluate(node => getComputedStyle(node).stroke)).not.toBe(originalFlowStroke)
  const flowPreview = appearance.locator('[data-prts-session-flow-preview]')
  await expect(flowPreview.locator('[data-prts-session-flow-preview-layer]')).toHaveCount(9)
  await expect(flowPreview).toHaveAttribute('data-prts-session-flow-preview-palette', 'amber')
  const customCoreColor = appearance.locator('[data-prts-setting-color][data-prts-setting-key="sessionFlowColorCore"]')
  await expect(customCoreColor).toHaveValue('#f0c800')
  await customCoreColor.evaluate(node => {
    node.value = '#8844cc'
    node.dispatchEvent(new Event('input', { bubbles: true }))
  })
  await expect(root).toHaveAttribute('data-prts-session-flow-palette', 'custom')
  await expect(root).toHaveCSS('--prts-session-flow-core', '#8844cc')
  await expect(flowPreview).toHaveAttribute('data-prts-session-flow-preview-palette', 'custom')
  await expect(appearance.locator('[data-prts-setting-color-output="sessionFlowColorCore"]')).toHaveText('#8844cc')
  await customCoreColor.evaluate(node => node.dispatchEvent(new Event('change', { bubbles: true })))

  const sessionSpeed = appearance.locator('[data-prts-setting-range][data-prts-setting-key="sessionFlowSpeed"]')
  await expect(root).toHaveAttribute('data-prts-session-flow-speed', '1')
  await sessionSpeed.evaluate(node => {
    node.value = '1.75'
    node.dispatchEvent(new Event('input', { bubbles: true }))
  })
  await expect(root).toHaveAttribute('data-prts-session-flow-speed', '1.75')
  await expect(appearance.locator('[data-prts-session-flow-speed-output]')).toHaveText('1.75× · 高速')
  await sessionSpeed.evaluate(node => node.dispatchEvent(new Event('change', { bubbles: true })))
...
test('keeps daylight appearance controls distinct and readable', async ({ page }) => {
  ...
  await expect(visualOptions).toHaveCount(26)
  ...
  const customCoreColor = settings.locator('[data-prts-setting-color][data-prts-setting-key="sessionFlowColorCore"]')
  await customCoreColor.evaluate(node => {
    node.value = '#80c040'
    node.dispatchEvent(new Event('input', { bubbles: true }))
  })
  await expect(page.locator('html')).toHaveAttribute('data-prts-session-flow-palette', 'custom')
  await expect(page.locator('[data-prts-session-row][aria-selected="true"] [data-prts-session-lifeline-filament="core"] [data-prts-session-lifeline-layer="body"]')).toHaveCSS('stroke', 'rgb(128, 192, 64)')
  await expect(settings.locator('[data-prts-session-flow-preview-filament="core"] [data-prts-session-flow-preview-layer="body"]')).toHaveCSS('stroke', 'rgb(128, 192, 64)')
  const daylightHalo = page.locator('[data-prts-session-row][aria-selected="true"] [data-prts-session-lifeline-filament="core"] [data-prts-session-lifeline-layer="halo"]')
  const daylightPreviewHalo = settings.locator('[data-prts-session-flow-preview-filament="core"] [data-prts-session-flow-preview-layer="halo"]')
  await expect(daylightHalo).toHaveCSS('opacity', '0.45')
  await expect(daylightHalo).toHaveCSS('filter', 'blur(1.05px)')
  await expect(daylightPreviewHalo).toHaveCSS('opacity', '0.45')
  await expect(daylightPreviewHalo).toHaveCSS('filter', 'blur(1.3px)')
})
